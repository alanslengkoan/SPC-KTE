...
<script>
    ...
    var untukTabelPemesanan = function() {
        tabelDataDt = $('#tabel-pembayaran').DataTable({
            ...
            pageLength: 10,
            ajax: '<?= distribusi_url() ?>pembayaran/get_data_dt',
            columns: [{
                    title: 'No.',
                    data: null,
                    ...
                    render: function(data, type, full, meta) {
                        if (full.status_invoice === '1') {
                            return `<div class="button-icon-btn button-icon-btn-cl">
                                        <a href="<?= distribusi_url() ?>pembayaran/invoice/` + btoa(full.id_penjualan) + `" class="btn btn-success btn-sm waves-effect"><i class="fa fa-dollar"></i>&nbsp;Invoice</a>
                                    </div>`;
                        } else {
                            return `-`;
                        }
