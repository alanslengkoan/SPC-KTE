...
<script>
    ...
    var untukTabelPembelian = function() {
        tabelDataDt = $('#tabel-pembelian').DataTable({
            ...
            pageLength: 10,
            ajax: '<?= admin_url() ?>pembelian/get_data_dt',
            columns: [{
                    title: 'No.',
                    data: null,
                ...
                },
                {
                    ...
                    render: function(data, type, full, meta) {
                        ...
                        if (full.status_approve === '1' && full.status_pembayaran === '1' && full.status_invoice === '1') {
                            html = `
                                <a href="<?= admin_url() ?>pembayaran/invoice/` + btoa(full.id_pembelian) + `" class="btn btn-primary btn-sm waves-effect"><i class="fa fa-dollar"></i>&nbsp;Invoice</a>&nbsp;
                                <a href="<?= admin_url() ?>pembelian/detail/` + btoa(full.id_pembelian) + `" class="btn btn-info btn-sm waves-effect"><i class="fa fa-info"></i>&nbsp;Detail</a>
                            `;
                        } else if (full.status_approve === '1' && full.status_pembayaran === '0' && full.status_invoice === '0') {
                            html = `
                                <a href="<?= admin_url() ?>pembayaran/add/` + btoa(full.id_pembelian) + `" class="btn btn-primary btn-sm waves-effect"><i class="fa fa-dollar"></i>&nbsp;Pembayaran</a>&nbsp;
                                <a href="<?= admin_url() ?>pembelian/detail/` + btoa(full.id_pembelian) + `" class="btn btn-info btn-sm waves-effect"><i class="fa fa-info"></i>&nbsp;Detail</a>
                            `;
                        } else {
                            html = `
