...
<script>
    ...
    var untukTabelBarang = function() {
        tabelDataDt = $('#tabel-stok-barang').DataTable({
            ...
            pageLength: 10,
            ajax: '<?= admin_url() ?>stok_barang/get_data_dt',
            columns: [{
                    title: 'No.',
                    data: null,
    ...
    var untukResetForm = function() {
        $(document).on('click', '#btn-add', function() {
            ...
            $('#inpkdbarang').val('');
            $('#detail-komposisi').html('');
        });
    }();

    let untukTambahDanUbahData = function() {
    ...
    let untukPilihBarang = function() {
        $(document).on('change', '#inpkdbarang', function() {
            ...
            $.ajax({
                type: "post",
                url: "<?= admin_url() ?>stok_barang/check_komposisi",
                dataType: 'html',
                data: {
                    id: ini.val(),
                },
                success: function(response) {
                    $('#detail-komposisi').html(response);
                }
            });
        });
    }();
